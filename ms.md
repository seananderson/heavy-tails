...
2. Plot showing time series we assumed were recorded as log transformed (figure removed).
...
3. Fig. \ref{fig:correlates} shows possible biological and time-series-property correlates of heavy-tailed behaviour. The model predictions are shown for two covariates which had high probability of non-zero relationships.
...
5. Percent increase in probability of observing heavy tails with increased time series length (Fig. \ref{fig:perc-inc-p}).
...
6. Stan code for the correlates of heavy tails modelling.
...
4. Are the observed frequencies by taxonomic class real or an observational phenomena? 
   Bring in dynamics-observation scale mismatch issue. Also note that variance and 
   heavy tails are not the same thing, in fact they are somewhat inversely related in 
   the same population.

5. Ways forward. Can we forecast the probability of black swans in space and time or 
   detect them quickly? Move from phenomenological to mechanistic (e.g.\ recruitment) 
   models. Investigation into the mechanisms and correlates in a subset of higher 
   resolution data. Hierarchical $\nu$ modelling.
