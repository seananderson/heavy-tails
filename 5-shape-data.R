...
gomp_hat_obs_0.2 <- plyr::join(gomp_hat_obs_0.2, lookup)
gomp_hat_logistic <- plyr::join(gomp_hat_logistic, lookup)
...
library(reshape2)
gomp_hat_base_long <- melt(gomp_hat_base, id.vars = c("main_id", "taxonomic_class", "taxonomic_order", "taxon_name"), measure.vars = c("log_sigma_proc_50", "dataset_length", "b_50", "lambda_50"))
