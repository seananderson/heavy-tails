...
Here, we assess the frequency and magnitude of black-swan dynamics across time
...
We examined all time series with published explanations of why black-swan
...
<!--TODO: Much of this paragraph needs to be reported, but since it does not
alter our conclusions, belongs in the supplementary materials or methods.-->
...
these likely influence the detection of heavy tails. For example, if we make
...
**Author Contributions**: S.C.A. and T.A.B. conceived the project; S.C.A.,
...
**Figure 3: Standardized probabilities and covariates of heavy-tailed
...
systematically changed our conclusions. These models allowed for
autocorrelation of the residuals, density independence, observation error, and
Ricker-logistic population dynamics. Furthermore, we tested the sensitivity of
our results to weaker and stronger priors on $\nu$ and used simulated data to
test how easily we could detect $\nu$ given different sample sizes and to
ensure we could recover unbiased parameter estimates.

We investigated possible covariates of heavy-tailed population dynamics with a
beta regression multilevel model with a logit link[@ferrari2004]. We propagated
uncertainty from the estimated Gompertz parameters as normal distributions
around these covariates. We accounted for broad patterns of phylogenetic
relatedness by letting the intercepts vary across taxonomic class, order, and
species level. We fit all models in a Bayesian framework using
Stan[@stan-manual2014] via the R computing environment[@r2014].
-->
...
We fit our models with Stan 2.4.0[@stan-manual2014], and R 3.1.1[@r2014]. We
...
**Alternative population models** We fit alternative population models to test
