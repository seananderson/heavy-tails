...
conservation strategies that are robust to such extremes.
...
for wild populations and is not simply integrated
...
We picked the t distribution for pragmatic and mechanistic reasons.
...
Mechanistically, the t distribution can be formed from a mixture of normal distributions
...
did not make substantial improvements to model bias (Fig.~S\ref{fig:sim-prob}).
...
population declines.
\textit{TODO: do we want to have a recap paragraph at the beginning of the discussion now?}
...
Given our results, we think modelers should routinely consider representing population dynamic
...
we suggest $\nu = 4$ or $5$ and possibly a
skewness parameter ($\gamma$) of $\sim 0.7$ (Fig.~4a)
