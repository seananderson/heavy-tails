...
sim_gomp_extinction <- function(abundance_ts, lambda = 1, b = 0.5, N = 100L,
  nu = 3, log_skew = 1, sigma_proc = 0.2, id = 1, 
  bootstrap_residuals = FALSE, model = "heavy") {
  ...
  if (bootstrap_residuals) {
    ...
    if (model == "heavy") {
      proc_error <- rskt(N, df = nu, gamma = exp(log_skew)) * sigma_proc
    } else {
  ...
  y <- gompertz(N, lambda, b, proc_error, y1)
    ...
    data.frame(b = b, N = N,
      nu = nu, sigma = sigma_proc, thresh = thresh, lambda = lambda,
      skew = exp(log_skew), dat = y_return, generation = generation, 
      main_id = id)
}
```
