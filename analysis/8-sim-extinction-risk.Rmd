...
```{r}
library("skewt")
sim_gomp_extinction <- function(abundance_ts, lambda = 1, b = 0.5, N = 200,
  gamma = 0.75, df = 3, thresh = 3, make_plot = FALSE, sigma = 0.2,
  return_dat = FALSE, id = 1) {

  y1 <- log(abundance_ts[length(abundance_ts)])
  ...
  if(make_plot) {
    ...
    y_return <- c(log(abundance_ts), y)
    generation <- c(seq(-length(abundance_ts) + 1, 0), seq_along(y))
    data.frame(b = b, N = N, gamma = gamma,
...
library("rstan")
id <- id_i
x <- readRDS(paste0("../scratch/base/sm-", id, ".rds"))
x <- rstan::extract(x)
gpdd <- readRDS("gpdd-clean.rds") # to get the last abundance
this_dat <- filter(gpdd, main_id == id)
...
p <- ggplot(out_df2, 
  ...
  facet_wrap(~main_id, scales = "free_y") 
print(p)
