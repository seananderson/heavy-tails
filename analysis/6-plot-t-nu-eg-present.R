...
for(i in seq_along(bs_cols)) {
  pdf(paste0("t-dists-", i, ".pdf"), width = 3, height = 2)
  par(xpd = NA)
  ...
  x <- seq(-8.5, 8.5, length.out = 500)
  plot(x, dt(x, 2), type = "n", xlab = "",
    ylab = "", yaxs = "i", lty = 1, , lwd = lwd,
    xlim = c(-8, 8), ylim = c(0, 0.42), col = cols[1], xaxt = "n", yaxt = "n", axes = FALSE)
  # text(-7.5, 0.393, quote((a)))
  # mtext("Student-t distribution", side = 3, line = 0, cex = 0.9)
  # axis(2, at = c(0, 0.2, 0.4), col = box.col)
  if(i == 1) lines(x, dt(x, 2), lty = 1, col = cols[1], lwd = lwd)
  if(i == 2) {
  ...
  if(i == 4) lines(x, dt(x, 20), lty = 1, col = cols[4], lwd = lwd)
  if(i == 5) {
    lines(x, dt(x, 1e9), lty = 1, col = cols[5], lwd = lwd)
  }
  ...
  # zoomed panel:
#   plot(x, dt(x, 2), type = "n", xlab = "Standard deviation",
#     ylab = "", yaxs = "i", lty = 1, , lwd = lwd,
#     xlim = c(-8, 8), ylim = c(0, 0.025), col = cols[1], yaxt = "n", xaxt = "n")
#   # text(-7.5, 0.0218, quote((b)))
#   # axis(2, at = c(0, 0.02), col = box.col)
#   if(i == 1) lines(x, dt(x, 2), lty = 1, col = cols[1], lwd = lwd)
#   if(i == 2) lines(x, dt(x, 3), lty = 1, col = cols[2], lwd = lwd)
#   if(i == 3) lines(x, dt(x, 5), lty = 1, col = cols[3], lwd = lwd)
#   if(i == 4) lines(x, dt(x, 20), lty = 1, col = cols[4], lwd = lwd)
#   if(i == 5) lines(x, dt(x, 1e6), lty = 1, col = cols[5], lwd = lwd)
#   par(xpd = NA)
