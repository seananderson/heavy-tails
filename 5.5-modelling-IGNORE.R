# this file fits GLMMs to the probability of heavy tails
# to examine correlates
# note that the glmmADMB models take a while to compile and fit
# there are remnants of an lme4 analysis commented out
...
#   i <<- i + 1
#   m1 <- glmer(heavy ~ gomp_hat_base[, param] + (1 | taxonomic_class /
#       taxonomic_order), family = binomial, data = gomp_hat_base)
#   models[[i]] <- m1
#   est <- fixef(m1)[[2]]
...
#m.all <- glmer(heavy ~ log_Lifesp_scaled + log_sigma_proc_50_scaled +
  #log_dataset_length_scaled + b_50_scaled + lambda_50_scaled + (1 |
    #taxonomic_class / taxonomic_order), family = binomial, data = gomp_hat_base)
...
m.admb.2 <- glmmadmb(p10 ~ log_Lifesp_scaled + log_sigma_proc_50_scaled +
  log_dataset_length_scaled + b_50_scaled + lambda_50_scaled + (1 |
    taxonomic_class / taxonomic_order), family = "beta", data = temp.dat,
  save.dir = "admb")
save(m.admb.2, file = "m.admb.2.rda")

temp.dat2 <- gomp_hat_base[,c("log_Lifesp_scaled", "log_sigma_proc_50_scaled",
  "dataset_length", "log_dataset_length_scaled", "b_50_scaled",
  "lambda_50_scaled", "taxonomic_class", "taxonomic_order", "heavy", "p10")]
...
m.admb.3 <- glmmadmb(heavy ~ log_Lifesp_scaled + log_sigma_proc_50_scaled +
  dataset_length + b_50_scaled + lambda_50_scaled + (1 | taxonomic_class /
    taxonomic_order), family = "binomial", data = temp.dat2)
save(m.admb.3, file = "m.admb.3.rds")

m.admb.4 <- glmmadmb(p10 ~ log_Lifesp_scaled + log_sigma_proc_50_scaled +
  dataset_length + b_50_scaled + lambda_50_scaled + (1 | taxonomic_class /
    taxonomic_order), family = "beta", data = temp.dat2)
...
par(mar = c(4, 1, 2, 1), cex = 0.9, oma = c(1, 4, 1, 1))

plot(x, plogis(p.beta$fit), type = "l", ylab = "", xlab = "Time-series length",
  ylim = c(0, 1.01), log = "x", yaxs = "i")
with(temp.dat2, points(dataset_length, p10, pch = 21, col = "#00000070",
    bg = "#00000030"))
polygon(c(x, rev(x)), c(plogis(p.beta$fit + qnorm(0.75) * p.beta$se.fit),
    rev(plogis(p.beta$fit - qnorm(0.75) * p.beta$se.fit))), border = NA,
  col = "#FF000020")
polygon(c(x, rev(x)), c(plogis(p.beta$fit + qnorm(0.975) * p.beta$se.fit),
    rev(plogis(p.beta$fit - qnorm(0.975) * p.beta$se.fit))), border = NA,
  col = "#FF000020")
par(xpd = NA)
mtext("Probability of\nobserving heavy tails", side = 2, line = 2.5, cex = 0.8,
  las = 0)
mtext("Beta model", side = 3, line = 1, cex = 0.8, las = 1)
par(xpd = FALSE)

plot(x, plogis(p$fit), type = "l", ylab = "", xlab = "Time-series length", ylim
  = c(0, 1), log = "x", yaxt = "n")
with(temp.dat2, points(dataset_length, jitter(heavy, 0.1), pch = 21, col =
    "#00000070", bg = "#00000030"))
polygon(c(x, rev(x)), c(plogis(p$fit + qnorm(0.75) * p$se.fit), rev(plogis(p$fit
        - qnorm(0.75) * p$se.fit))), border = NA, col = "#FF000020")
polygon(c(x, rev(x)), c(plogis(p$fit + qnorm(0.975) * p$se.fit),
    rev(plogis(p$fit - qnorm(0.975) * p$se.fit))), border = NA, col =
  "#FF000020")
...
#m.all2 <- glmer(heavy ~ log10_Len_scaled + log_sigma_proc_50_scaled +
  #log_dataset_length_scaled + b_50_scaled + lambda_50_scaled + (1 |
    #taxonomic_class / taxonomic_order), family = binomial, data = gomp_hat_base)
...
# MCMC: (takes a long time, and needs more iterations than this)
#m.admb.5 <- glmmadmb(heavy ~ log_Lifesp_scaled + log_sigma_proc_50_scaled +
  #dataset_length + b_50_scaled + lambda_50_scaled + (1 | taxonomic_class /
    #taxonomic_order), family = "binomial", data = temp.dat2, mcmc = TRUE)
#m <- as.mcmc(m.admb.5$mcmc)
#library(scapeMCMC)
#plotTrace(m)
#
#
