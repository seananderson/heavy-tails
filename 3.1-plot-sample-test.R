...
p <- ggplot(sample_t, aes(iter, 1/med_nu)) + geom_pointrange(aes(ymin = 1/l_nu, ymax = 1/u_nu)) +
  ...
  ggsave("t-dist-sampling-sim-prior-exp0.01.pdf", width = 8, height = 5)

p <- ggplot(sample_t, aes(iter, med_sigma)) + geom_pointrange(aes(ymin = l_sigma, ymax = u_sigma)) +
  facet_grid(nu_true~N) +
  geom_hline(aes(yintercept = 1), col = "red") +
  xlab("Iteration") + ylab("Scale parameter")

  ggsave("t-dist-sampling-sim-sigma-prior-exp0.01.pdf", width = 8, height = 5)
