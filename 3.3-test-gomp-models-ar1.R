...
sim_gomp <- function(id = 99, nu = 10, sigma_proc = 0.6, phi = 0.4,
  sigma_obs_true = 0.001, sigma_obs_assumed = 0.001, N = 50, b = 0.2,
  lambda = 3, y1 = 3, iterations = 2000, warmup = 1000, plot = FALSE,
  max_iter = 8000, chains = 4, seed) {

  file_base <- paste0("/global/scratch/anderson/heavy/sim3-AR1/sm-",
  ...
  set.seed(seed)
  proc_error <- rep(NA, N)
  proc_error[1] <- metRology::rt.scaled(1, df = nu, mean = 0, sd = sigma_proc)
  for(i in 2:N) {
    proc_error[i] <- phi * proc_error[i-1] +
      metRology::rt.scaled(1, df = nu, mean = 0, sd = sigma_proc)
  }

  y[1] <- y1
  y[2] <- lambda + b * y[1] + proc_error[1]
  for(i in 3:N) {
    y[i] <- lambda + b * y[i-1] + proc_error[i-1] +
      phi * (y[i-1] - (lambda + b * y[i-2]))
  }
  ...
  if(!file.exists(paste0(file_base, ".rds"))) {
    while((max_rhat > 1.25 | min_neff < 10) & iterations <= max_iter) {
      if(sigma_obs_assumed <= 0.01) { # use the faster Stan model without obs error
        sm <- sampling(stan_gomp_ar1,
          data = list(N = N, y = y, nu_rate = 0.01,
            b_lower = -1, b_upper = 2),
          pars = c("lambda", "sigma_proc", "nu", "b", "phi"),
          iter = iterations,
  ...
  out <- data.frame(id = id, m, l, u, p_0.1 = p_0.1, p_0.2 = p_0.2,
    max_rhat = max_rhat, min_neff = min_neff, N = N, phi_true = phi,
...
check_nu_1 <- plyr::ldply(iters, function(i) sim_gomp(i, nu = 3,
  seed = nu_3_seeds_N50$seeds[i], iterations = 600, warmup = 300, chains = 3))
check_nu_2 <- plyr::ldply(iters, function(i) sim_gomp(i, nu = 5,
  seed = nu_5_seeds_N50$seeds[i], iterations = 600, warmup = 300, chains = 3))
...
check_nu_2 <- plyr::ldply(iters, function(i) sim_gomp(i, nu = 5,
  seed = nu_5_seeds_N100$seeds[i], N = 100, iterations = 500, warmup = 200, chains = 3))
check_nu_4 <- plyr::ldply(iters, function(i) sim_gomp(i, nu = 1e9,
  seed = nu_norm_seeds[i], N = 100,iterations = 500, warmup = 200, chains = 3))
...
saveRDS(check_nu, file = "check_nu_ar1.rds")
