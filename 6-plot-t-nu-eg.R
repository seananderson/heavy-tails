...
box.col <- "grey20"
pdf("t-nu-eg.pdf", width = 7.5, height = 4)
...
mtext("Standard deviations", side = 1, outer = FALSE, line = 2.5, las = 0, cex = 0.9)
#dev.off()
par(xpd = FALSE)

load("nu_effective_seeds.rda")

make_ts <- function(seed, nu, ylim = c(-4.5, 6.0), bs_col = "red") {
  N <- 50
  set.seed(seed)
  ...
  pch = rep(21, N)
  pch[black_swan_i] <- 19
  plot(y, type = "l", ylim = ylim, ylab = "", xlab = "", yaxt = "n", xaxt = "n", cex = 0.8, col = "grey20", xaxs = "i")
  points(seq_along(y), y, pch = pch, cex = cex, col = col, bg = "grey70")
  return(list(y = y, nu = nu))
}

seed <- nu_5_seeds_N50$seeds[17]
#par(mfrow = c(3, 1))
y1 <- make_ts(seed = seed, nu = 3, bs_col = bs_cols[2])
text(1, 5.4, quote(widehat((c)~nu==3)), pos = 4)
...
text(1, 5.4, quote(widehat((d)~nu==5)), pos = 4)
y3 <- make_ts(seed = seed, nu = 1e9, bs_col = "darkgrey")
axis(1, at = c(1, seq(10, 50, 10)), col = box.col)
text(1, 5.4, quote(widehat((e)~Normal)), pos = 4)
mtext("Time step", side = 1, line = 2.5, las = 0, cex = 0.9)
...
sm1 <- sampling(stan_gomp,
  data = list(N = 50, y = y1$y, nu_rate = 0.01,
    b_lower = -1, b_upper = 2),
  pars = c("lambda", "sigma_proc", "nu", "b"), iter = 1000,
  chains = 4, warmup = 500)
...
# breaks <- 1/seq(2, 1000, 4)

# make_hist <- function(x, type = "log", ylim = NULL) {
#   if(type == "log") {
#     breaks <- seq(log(2), log(1000), length.out = 50)
#     hist(log(x), xaxt = "n", xlim = log(c(2, 500)), breaks = breaks, main = "")
#     axis(1, at = log(c(2, 5, 10, 50, 100)), labels = c(2, 5, 10, 50, 100))
#   }
#   if(type == "inverse") {
#     breaks <- seq(1/2, 1/1000, length.out = 50)
#     hist(1/x, xaxt = "n", xlim = 1/c(2, 500), breaks = breaks, main = "")
#     axis(1, at = 1/c(2, 5, 10, 50, 100), labels = c(2, 5, 10, 50, 100))
#   }
#   if(type == "identity") {
#     breaks <- seq(2, 1000, length.out = 800)
#     if(is.null(ylim)) {
#       hist(x, xaxt = "n", xlim = c(2, 60), breaks = breaks, main = "", yaxt = "n", ylab = "", xlab = "")
#     } else {
#       hist(x, xaxt = "n", xlim = c(2, 60), breaks = breaks, ylim = ylim, main = "", yaxt = "n", ylab = "", xlab = "")
#     }
#     axis(1, at = c(2, seq(20, 60, 20)), labels = c(2, seq(20, 60, 20)))
#   }
# }
#par(mfrow = c(3, 1))
#make_hist(e1)
...
#make_hist(e1, type = "inverse")
#make_hist(e2, type = "inverse")
#make_hist(e3, type = "inverse")

#make_hist(e1, type = "identity", ylim = c(0, 1400))
#make_hist(e2, type = "identity", ylim = c(0, 1400))
#make_hist(e3, type = "identity", ylim = c(0, 1400))

make_hist2 <- function(x) {
  h <- hist(x, breaks = seq(2, max(x)+20,2),
    xlim = c(2, 50), xlab = "nu hat", xaxt = "n", main = "", col = "white",
  ...
  segments(quantile(x, probs = 0.25), ylim[2] / 2, quantile(x, probs = 0.75), ylim[2] / 2, col = "grey10")
  points(median(x), ylim[2] / 2, pch = 21, col = "grey10", bg = "white")
  z <- seq(2, 50, length.out = 200)
  par(new = TRUE)
  plot(z, dexp(z, 0.01), type = "l", ylim = c(0, 0.025), xlab = "", ylab = "", axes = FALSE, xlim = xlim, xaxs = "i", lty = 2)
  par(new = FALSE)
}
#par(mfrow = c(3, 1))
make_hist2(e1)
text(2.5, 0.0235, "(f) Heavy tails", pos = 4)
text(30, 0.009, "Prior", pos = 4)
mtext("Model fits", side = 3, line = 0, cex = 0.9)
make_hist2(e2)
text(2.5, 0.0235, "(g) Slightly heavy tails", pos = 4)
mtext("Samples or probability", side = 2, line = 0.5, las = 0, cex = 0.9)
text(10, 0.0025, "Posterior samples", pos = 4)
make_hist2(e3)
axis(1, at = c(2, seq(10, 40, 10)), col = box.col)
mtext(quote(widehat(nu)), side = 1, line = 2.5, las = 0, cex = 0.9)
text(22, 0.015, "Median + IQR", pos = 4)
text(2.5, 0.0235, "(h) Normal tails", pos = 4)
dev.off()
