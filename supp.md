...
To fit the Ricker-logistic models, we chose a prior on $K$ uniform between zero and twice the maximum observed abundance (@clark2010 chose uniform between zero and maximum observed, which is more informative). We set the prior on $r_\mathrm{max}$ as uniform between 0 and 20 as in @clark2010. We used the same priors on $\nu$ and $\sigma$ as in the Gompertz model.
